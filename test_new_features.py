...
def test_feishu_folder():
    ...
    assert "飞书" in folder_names, f"缺少 飞书 键: {folder_names}"
...
@test("permissions.request 已从 background.js 移至 options.js（用户手势上下文）")
def test_permissions_request_moved_to_options():
    bg_path = os.path.join(os.path.dirname(__file__), "extension", "background.js")
    with open(bg_path, "r", encoding="utf-8") as f:
        bg_content = f.read()
    ...
    # permissions.request 不应作为代码调用出现在 service worker 中（注释说明除外）
    import re
    code_calls = re.findall(r'^\s*(?!.*//.*permissions\.request).*permissions\.request\s*\(', bg_content, re.MULTILINE)
    assert len(code_calls) == 0, \
        "chrome.permissions.request() 不应在 background.js (service worker) 中调用"
    # 应在 options.js 中（有用户手势上下文）
    assert "permissions.request" in opts_content, \
        "chrome.permissions.request 应在 options.js 中调用"
...
@test("风险: chrome.permissions.request 已从 service worker 中移除")
def test_risk_permissions_not_in_sw():
    """
    chrome.permissions.request() 只能在用户手势上下文中调用。
    已从 background.js (service worker) 中移除，移至 options.js。
    验证 background.js 中不再包含 permissions.request。
    """
    bg_path = os.path.join(os.path.dirname(__file__), "extension", "background.js")
    with open(bg_path, "r", encoding="utf-8") as f:
        bg = f.read()
    # 确认 permissions.request 作为代码调用已从 service worker 中移除（注释说明除外）
    import re
    code_calls = re.findall(r'^\s*(?!.*//.*permissions\.request).*permissions\.request\s*\(', bg, re.MULTILINE)
    assert len(code_calls) == 0, \
        "permissions.request() 调用仍在 background.js 中，应移至 options.js"
