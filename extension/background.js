...
// 从本地存储恢复用户自定义端口（返回 Promise 以便等待完成）
const _portReady = new Promise((resolve) => {
...
async function registerDiscourseContentScripts(domains) {
    ...
    try {
        ...
        // 依赖 manifest.json 中 optional_host_permissions 声明 + 用户在 options 页面授权
        // 这里直接尝试注册内容脚本，已授权的域名会生效
